...
DetourXS::DetourXS():
	m_lpFuncOrig(nullptr), m_lpFuncDetour(nullptr),
	m_lpbFuncOrig(nullptr), m_lpbFuncDetour(nullptr),
	m_OrigJmp(Relative), m_TrampJmp(Relative),
	m_detourLen(0), m_Created(FALSE)
{
}
...
LPVOID DetourXS::RecurseJumps(LPVOID lpAddr)
{
	...
	#else
		lpDest = reinterpret_cast<LPVOID>(uAddr(*PDWORD(lpbAddr + 1)) + uAddr(lpbAddr) + relativeJmpSize
...
void DetourXS::WriteJump(const LPBYTE lpbFrom, const LPBYTE lpbTo, JmpType jmpType)
{
	...
	else if(jmpType == Relative)
	{
		// E9 [to - from - jmp_size]
		lpbFrom[0] = 0xE9;
		DWORD offset = DWORD(uAddr(lpbTo) - uAddr(lpbFrom)) - relativeJmpSize;
